...
const AddDoctor = () => {
    ...
    const { backendUrl } = useContext(AdminAppContext)
    ...
    const onSubmitHandler = async (event) => {
        ...
        if (isInspectionMode) {
            return toast.info('View only — Inspection Mode')
        }
    ...
    return (
        <form onSubmit={onSubmitHandler} className='m-5 w-full'>
            ...
            <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
                ...
                <button type='submit' 
                        disabled={isInspectionMode}
                        className={`px-10 py-3 mt-4 text-white rounded-full ${isInspectionMode ? 'bg-gray-400 cursor-not-allowed opacity-70' : 'bg-primary cursor-pointer'}`}
                        title={isInspectionMode ? 'View only — Inspection Mode' : ''}
                >
                    Add doctor
                </button>
